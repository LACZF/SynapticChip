...
//    time    tRES;
    ...
    begin : memory_initialize
        reg [31:0] temp_array[0:TOP_Add/4];
        integer j;

        for ( i = 0; i <=  TOP_Add; i = i + 1 )
            ARRAY[i] = 8'hff;
        if ( Init_File != "none" ) begin
            $readmemh(Init_File,temp_array) ;
            for (j = 0; j <= TOP_Add/4; j = j + 1) begin
                if (j*4 <= TOP_Add) ARRAY[j*4]   = temp_array[j][31:24];
                if (j*4+1 <= TOP_Add) ARRAY[j*4+1] = temp_array[j][23:16];
                if (j*4+2 <= TOP_Add) ARRAY[j*4+2] = temp_array[j][15:8];
                if (j*4+3 <= TOP_Add) ARRAY[j*4+3] = temp_array[j][7:0];
            end
        end
