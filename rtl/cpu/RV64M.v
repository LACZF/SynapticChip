...
module RV64M (
	...
	always @(*) begin
		// 计算各种乘法结果，以备后续使用
		mul_result = {{32{in_0_i[31]}}, in_0_i} * {{32{in_1_i[31]}}, in_1_i}; // 有符号×有符号
		mulhsu_result = {{32{in_0_i[31]}}, in_0_i} * {32'b0, in_1_i}; // 有符号×无符号
		mulhu_result = {32'b0, in_0_i} * {32'b0, in_1_i}; // 无符号×无符号

		case (op_i)
			`ALU_OP_MUL : begin // 乘法（结果低32位）
				out_o = mul_result[31:0];
			end
			`ALU_OP_MULH : begin // 高位乘法（有符号×有符号）
				out_o = mul_result[63:32];
			end
			`ALU_OP_MULHSU : begin // 高位乘法（有符号×无符号）
				out_o = mulhsu_result[63:32];
			end
			`ALU_OP_MULHU : begin // 高位乘法（无符号×无符号）
				out_o = mulhu_result[63:32];
			end
			`ALU_OP_DIV : begin // 有符号除法
				if (in_1_i == 0) begin
					// 除以零，根据RISC-V规范返回全1
					out_o = {`WORD_DATA_W{1'b1}};
				end else if (in_0_i == {1'b1, {31{1'b0}}} && in_1_i == {32{1'b1}}) begin
					// -2^31 / -1，根据RISC-V规范返回-2^31
					out_o = {1'b1, {31{1'b0}}};
				end else begin
					out_o = $signed(in_0_i) / $signed(in_1_i);
				end
			end
