...
module cache_system (
    ...
    // ==================== 缓存参数配置 ====================
    parameter ICACHE_SIZE = 8192;      // 8KB指令缓存
    parameter DCACHE_SIZE = 8192;      // 8KB数据缓存
    parameter L2CACHE_SIZE = 32768;    // 32KB L2缓存

    parameter ICACHE_LINE_SIZE = 16;   // 16字节缓存行
    parameter DCACHE_LINE_SIZE = 16;
    parameter L2CACHE_LINE_SIZE = 32;  // 32字节缓存行

    parameter ICACHE_ASSOC = 2;        // 2路组相联
    parameter DCACHE_ASSOC = 2;
    parameter L2CACHE_ASSOC = 4;       // 4路组相联
    ...
    wire l2_arb_request_icache;
    wire l2_arb_request_dcache;
