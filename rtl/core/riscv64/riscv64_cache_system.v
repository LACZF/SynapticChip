// cache_system_64bit.v
module cache_system_64bit (
    input wire clk,
    input wire rst_n,

    // CPU接口
    input wire [63:0] cpu_imem_addr,
    output wire [63:0] cpu_imem_data,
    input wire cpu_imem_req,
    output wire cpu_imem_ack,

    input wire [63:0] cpu_dmem_addr,
    input wire [63:0] cpu_dmem_data_out,
    output wire [63:0] cpu_dmem_data_in,
    input wire cpu_dmem_we,
    input wire [7:0] cpu_dmem_sel,
    ...
    // ==================== 64位缓存参数配置 ====================
    parameter ICACHE_SIZE = 16384;      // 16KB指令缓存
    parameter DCACHE_SIZE = 16384;      // 16KB数据缓存
    parameter L2CACHE_SIZE = 131072;    // 128KB L2缓存

    parameter ICACHE_LINE_SIZE = 32;    // 32字节缓存行
    parameter DCACHE_LINE_SIZE = 32;
    parameter L2CACHE_LINE_SIZE = 64;   // 64字节缓存行

    parameter ICACHE_ASSOC = 4;         // 4路组相联
    parameter DCACHE_ASSOC = 4;
    parameter L2CACHE_ASSOC = 8;        // 8路组相联

    // ==================== 内部信号定义 ====================

    // L1缓存到L2缓存的接口
    wire [63:0] l2_req_addr;
    wire [63:0] l2_req_data;
    wire [63:0] l2_resp_data;
    wire l2_req_we;
    wire [7:0] l2_req_sel;
    wire l2_req_valid;
    wire l2_resp_valid;
    ...
    // L2缓存仲裁器
    cache_arbiter_64bit l2_arbiter (
        .clk(clk),
        .rst_n(rst_n),
    ...
    assign l2_req_sel = l2_arb_grant_dcache ? dcache_req_sel : 8'hFF;
