...
module uart_ring_node #(
    ...
    input       [DATA_WIDTH-1:0]         uart_data_out_i,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                `STATE_IDLE: begin
                    ...
                    if (int_pending) begin
                        // There is an interrupt pending, send interrupt message
                        state <= `STATE_ARB;
                        ring_out_valid_o <= 1'b1;
                        ring_out_src_o <= node_id_i;
                        ring_out_dest_o <= 0; // Send to main controller
                        ring_out_addr_o <= `REG_IIR;
                        ring_out_data_o <= {DATA_WIDTH{1'b1}}; // Interrupt identifier
                        ring_out_we_o <= 1'b1;
                        int_pending <= 1'b0;
                        ...
                        if (is_for_me) begin
                            // Data is for UART of this node
                            state <= `STATE_DATA;
                            uart_req_o <= 1'b1;
                            uart_addr_o <= ring_in_addr_i;
                            uart_data_in_o <= ring_in_data_i;
                            uart_we_o <= ring_in_we_i;
                ...
                `STATE_DATA: begin
                    // UART data processing state
                    if (uart_ack_i) begin
                        uart_req_o <= 1'b0;

                        if (!we_buffer) begin
                            // Read operation completed, prepare to send reply
                            data_buffer <= uart_data_out_i;
                            state <= `STATE_ARB;
                        end else begin
            ...
            // Detect interrupt
            if (uart_int_i && !int_pending) begin
                int_pending <= 1'b1;
            end
        end
    end
